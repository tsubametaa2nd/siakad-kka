...
export const assignmentRoutes = new Elysia({ prefix: "/assignments" })
  ...
  .guard({ beforeHandle: requireRole("teacher") }, (app) =>
    app
      ...
      .post("", async ({ user, body }) => ok(await assignmentsService.createAssignment(user.id, body as any)), { body: createAssignmentSchema })
      .put("/:id", async ({ user, params, body }) => ok(await assignmentsService.updateAssignment(user.id, params.id, body as any)), { body: updateAssignmentSchema })
